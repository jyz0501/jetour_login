...
def make_request(config, command_name, custom_params=None):
    ...
    if access_token:
        for key, value in params.items():
            if isinstance(value, str):
                if '${{ secrets.JETOUR_ACCESS_TOKEN }}' in value:
                    params[key] = access_token
                elif 'access_token' in value and not value.startswith('$'):
                    params[key] = access_token
    if task_id:
        for key, value in params.items():
            if isinstance(value, str):
                if '${{ secrets.JETOUR_TASK_ID }}' in value:
                    params[key] = task_id
                elif 'taskId' in value and not value.startswith('$'):
                    params[key] = task_id
